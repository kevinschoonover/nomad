...
func TestHTTP_ACLPolicyList(t *testing.T) {
	...
	httpACLTest(t, nil, func(s *TestAgent) {
		...
		for _, srv := range s.Servers {
			obj, err := srv.ACLPoliciesRequest(respW, req)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestHTTP_ACLPolicyQuery(t *testing.T) {
	...
	httpACLTest(t, nil, func(s *TestAgent) {
		...
		// Make the request
		for _, srv := range s.Servers {
			obj, err := srv.ACLPolicySpecificRequest(respW, req)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestHTTP_ACLTokenBootstrap(t *testing.T) {
	...
	httpTest(t, conf, func(s *TestAgent) {
		...
		// Make the request
		for _, srv := range s.Servers {
			obj, err := srv.ACLTokenBootstrap(respW, req)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestHTTP_ACLTokenList(t *testing.T) {
	...
	httpACLTest(t, nil, func(s *TestAgent) {
		...
		// Make the request
		for _, srv := range s.Servers {
			obj, err := srv.ACLTokensRequest(respW, req)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestHTTP_ACLTokenQuery(t *testing.T) {
	...
	httpACLTest(t, nil, func(s *TestAgent) {
		...
		// Make the request
		for _, srv := range s.Servers {
			obj, err := srv.ACLTokenSpecificRequest(respW, req)
			if err != nil {
				t.Fatalf("err: %v", err)
			}
...
func TestHTTP_OneTimeToken(t *testing.T) {
	...
	httpACLTest(t, nil, func(s *TestAgent) {
		...
		for i, srv := range s.Servers {
			obj, err := srv.UpsertOneTimeToken(respW, req)
			require.NoError(t, err)
			...
			obj, err = s.Servers[i].ExchangeOneTimeToken(respW, req)
			require.NoError(t, err)
			require.NotNil(t, obj)
